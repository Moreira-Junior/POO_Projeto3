...
public class Fachada {
	...
	public static Convidado criarConvidado(String email,String nome, int idade, String empresa) throws Exception {
		...
		if(empresa == null || empresa.isBlank()) {
			throw new Exception("Inserir empresa válida");
		}
	...
	public static Evento criarEvento (String data, String descricao, double preco) throws Exception {
		...
		Evento evento;
		if(preco < 0) {
			throw new Exception("Não é possível criar evento com valor negativo");
		}
		//localizar Evento no repositorio, usando a data 
		evento = repositorio.localizarEvento(data);
		if (evento!=null)
			throw new Exception("Não criou evento na data: " + data + " ja cadastrado(a)");
		
		
		//gerar id no repositorio
		int id = repositorio.getEventos().stream().mapToInt(e -> e.getId()).max().orElse(0);
		//criar evento
		evento = new Evento (id +1, descricao, data, preco);
	...
	public static void adiarEvento(String data, String novadata) throws Exception	{
		...
		Evento ev = repositorio.localizarEvento(data);
		if(ev == null) 
			throw new Exception("Não há evento nessa data: evento na data" + data + " inexistente");		
		//localizar evento no repositorio, usando novadata
		Evento evNovo = repositorio.localizarEvento(novadata);
		if(evNovo != null) 
			throw new Exception("Já há evento nessa data: evento na data" + novadata + " já existente");	
	...
	public static void 	apagarParticipante(String nome) throws Exception {
		...
		for(Evento evento : repositorio.getEventos()) {
			if(evento.localizar(nome) != null) {
				throw new Exception("Participante " + nome + " não pode ser removido, "
						+ "pois participa do evento " + evento.getDescricao());
			}
		}
